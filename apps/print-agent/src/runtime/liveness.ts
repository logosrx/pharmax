// Print-agent liveness marker.
//
// Like the worker, the print-agent is a polling drain with no HTTP socket,
// so ECS/Fargate health-checks it by testing for a marker file:
//
//   test -f /tmp/pharmax-print-agent-alive   (infra/terraform/modules/ecs/main.tf)
//
// Nothing previously created that file, so the container's health check
// could never pass and ECS would kill the task. This heartbeat:
//
//   1. Writes the marker once the print-job poll loop is running, so the
//      container becomes healthy as soon as it has actually booted.
...
// touching the real filesystem. This mirrors apps/worker/src/runtime/
// liveness.ts by design — both apps already keep app-local copies of the
// poll-loop runtime for the same reason (no cross-app runtime coupling).
 ...
 * print-agent task definition in `infra/terraform/modules/ecs/main.tf`.
 */
export const PRINT_AGENT_LIVENESS_MARKER_PATH = "/tmp/pharmax-print-agent-alive";

const DEFAULT_HEARTBEAT_MS = 10_000;

export interface LivenessHeartbeatOptions {
  readonly logger: Logger;
  /** Override the marker path. Defaults to {@link PRINT_AGENT_LIVENESS_MARKER_PATH}. */
...
export function createLivenessHeartbeat(options: LivenessHeartbeatOptions): LivenessHeartbeat {
  const filePath = options.filePath ?? PRINT_AGENT_LIVENESS_MARKER_PATH;
  ...
  async function touch(): Promise<void> {
    try {
      ...
      // not be able to crash the print-agent process.
  ...
  return {
    async start(): Promise<void> {
      ...
      // own — shutdown is gated by the poll loop, not this timer.
      timer.unref();
    },
