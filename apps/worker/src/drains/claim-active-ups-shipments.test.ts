...
function makeClient(rows: ReadonlyArray<Record<string, unknown>>): UpsShipmentClaimClient {
  return {
    $queryRaw: vi.fn().mockResolvedValue(rows),
  } as unknown as UpsShipmentClaimClient;
}

describe("claimActiveUpsShipments", () => {
  it("returns an empty array when no shipments are due", async () => {
    const client = makeClient([]);
    const result = await claimActiveUpsShipments(client, {
      batchSize: 50,
  ...
  it("passes batchSize + staleThresholdMs to the query as bound parameters", async () => {
    const client = makeClient([]);
    await claimActiveUpsShipments(client, { batchSize: 25, staleThresholdMs: 7_200_000 });
