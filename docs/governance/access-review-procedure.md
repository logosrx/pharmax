...
| 1.3     | 2026-08-19 | CTO    | §5.5 now requires the signed artifact's SHA-256 in the [evidence digest ledger](../compliance/evidence-digest-ledger.md). Surfaced by the first signed review: `evidence/` is gitignored, so nothing established which version of the worksheet had been signed. (Version 1.2 is [#211](https://github.com/logosrx/pharmax/pull/211), in flight at the time of writing — numbered around it rather than colliding with it.)              |
