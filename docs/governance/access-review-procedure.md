...
- Dependency-tree review (R-010)
- Secret-rotation cadence and drill (R-011)
- PHI-shape sampling of log and Sentry content (R-018)
- Public-source citation sampling on merged PRs (R-021)
- Device-hygiene attestation (R-014)

The last five were added 2026-08-20. Each was a **standalone quarterly
commitment in the risk register**, which contradicted this section's own
principle: four extra dated promises an assessor can find unkept, for work
that is the same walk through the same reports.

The device-hygiene item is a different case and worth naming separately.
R-014 already claimed, in its current-controls column, that _"quarterly
access reviews include a device-hygiene attestation question."_ They did
not. That is a control credited in the register and absent from the
procedure it credits — the R-027 pattern applied to governance rather than
to code. Adding it here is what makes the register's existing claim true,
rather than adding a new commitment.
...
7. **Null-approver break-glass check.** List `break_glass_session` rows opened in the period with `approvedByUserId IS NULL` and confirm each was genuinely a single-operator situation rather than a skipped approval.
8. **Device-hygiene attestation.** For every principal in the review, confirm full-disk encryption is on and the lock-screen timeout meets [AUP](../policies/acceptable-use-policy.md) §4. There is no MDM, so this is an attestation and not a measurement — record it as one. Satisfies the control R-014 already claims.
9. **Dependency-tree review.** Walk the current dependency tree for packages that are unmaintained, newly transitively introduced, or carrying an unactioned advisory. CI already blocks critical CVEs continuously; this is the periodic look at what CI does not fail on (R-010).
10. **Secret-rotation cadence.** Confirm each secret class is inside its rotation window per [secrets management](../security/secrets-management.md) §5, and that the rotation procedure has been exercised at least once in the period (R-011).
11. **PHI-shape sampling.** Sample recent log and Sentry content against the PHI patterns in `@pharmax/platform-core`'s redaction module. R-018's worst residual is that **nothing would detect** an exposure, and § 164.404(a)(2) starts the breach clock at when it _would have been known_ — so a periodic look is not optional diligence, it is what makes the clock start honestly.
12. **Public-source citation sampling.** Sample merged PRs for public-source citations on substantive design changes, per the clean-room policy (R-021). Four-eyes is required whenever a second engineer exists, and the code permits a null approver so that the emergency tool stays usable when one does not — see the [break-glass runbook](../compliance/break-glass-runbook.md) single-operator exception and R-029. Reviewing these is what keeps the exception an exception; unreviewed, "no approver was available" becomes the default explanation for every session.
...
| 1.4     | 2026-08-20 | CTO    | Folded five standalone quarterly commitments into this review as cross-checks §5.3.8 through §5.3.12: device hygiene (R-014), dependency tree (R-010), secret rotation (R-011), PHI-shape sampling (R-018) and public-source citation sampling (R-021). Four were separate dated promises for work that is the same walk through the same reports, contradicting §1.1. The fifth, device hygiene, was a control the register already CLAIMED this review performed and it did not — adding it makes an existing claim true rather than adding a commitment. |
