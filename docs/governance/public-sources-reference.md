...
| HL7 FHIR R4 — `AllergyIntolerance`                                  | <https://hl7.org/fhir/R4/allergyintolerance.html>                                              | Canonical model for allergy capture. Supplies `category`, `type`, `criticality`, `clinicalStatus`, `verificationStatus`, `reaction.severity` and `reaction.manifestation` — the vocabulary of the `patient_allergy` table, the `Allergy*` Prisma enums, and the screening engine's `RecordedAllergy`. Also the source for treating "no known allergy" as an assertion needing an asserter and a date, which Pharmax models as its own `patient_allergy_history_assertion` record rather than as an `AllergyIntolerance` with a negative code.                     |
| HL7 FHIR R4 — `MedicationKnowledge`                                 | <https://hl7.org/fhir/R4/medicationknowledge.html>                                             | Shape of the `DrugKnowledgeSource` seam in `@pharmax/clinical-screening`: a medication decomposes into ingredients and carries classification and dosing guidance. Pharmax ships the interface and no drug data.                                                                                                                                                                                                                                                                                                                                                  |
| RxNorm (NLM) — NDC → RXCUI → ingredient                             | <https://www.nlm.nih.gov/research/umls/rxnorm/index.html>                                      | The public, non-vendor route from a dispensed NDC to its active ingredients, and therefore the only clean-room path to ingredient-level allergy screening. **Not yet implemented** — the seam is the `DrugKnowledgeSource` interface in `@pharmax/clinical-screening`, and `SCREENING_AXIS_CAPABILITY` in `packages/verification/src/screening/axis-capability.ts` records what remains unsupported until an adapter lands. Recorded here because it is the intended source and any future adapter must be traceable to it rather than to a licensed vendor file. |
| HL7 FHIR R4 — `DetectedIssue`                                       | <https://hl7.org/fhir/R4/detectedissue.html>                                                   | Canonical model for a clinical issue detected about a patient's care; informs the finding shape and the `severity` value set (`high`/`moderate`/`low`) in `@pharmax/clinical-screening`.                                                                                                                                                                                                                                                                                                                                                                          |
...
### Licensed clinical knowledge bases

Drug-drug interaction tables, cross-sensitivity groupings, therapeutic classifications and dose ranges are **licensed proprietary content**. They are not a public source and must never be transcribed into this repository — not the data, and not the vendor's severity taxonomy, which is itself an editorial work product.

Pharmax therefore holds **no drug knowledge**. `@pharmax/clinical-screening` defines a `DrugKnowledgeSource` interface and ships only an empty in-memory implementation that the caller seeds; every fact it can return is one the deployment supplied. A customer's licence to a clinical database is satisfied by an adapter implementing that interface, kept outside this package so the licence boundary is visible in the dependency graph rather than buried in a data file.

The consequence for design review: a PR that adds interaction facts, class groupings or dose ranges to this repository is a clean-room incident regardless of where the contributor believes they came from, and should be refused. Illustrative fixtures in tests use obviously synthetic codes (`ING_ALFA`) for exactly this reason — they cannot be mistaken for clinical content, by a reader or by a reviewer.
...
| PV1 clinical screening (`@pharmax/clinical-screening`)           | HL7 FHIR `DetectedIssue` (finding shape, severity value set); HL7 FHIR `AllergyIntolerance` (allergy vocabulary); HL7 FHIR `MedicationKnowledge` (knowledge-seam shape). Drug interaction, cross-sensitivity and dose-range FACTS are **not** sourced here — see §6, "Licensed clinical knowledge bases." |
