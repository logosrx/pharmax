...
- **Known caveat.** See `kms2` in
  [`code-evidence-map.md`](../soc2/code-evidence-map.md) §6 for a
  documented limitation: the adapter currently pins `KeyId` in
  `kms.Decrypt`, which blocks reading envelopes wrapped under a
  previous CMK identity after an alias-swap rotation. The runbook
  procedure works for AWS-side automatic key-material rotation;
  manual CMK identity rotation requires the fix in `kms2`. Routine
  posture is unaffected because automatic rotation handles 99% of
  rotation events.
...
5. **CMK aliases are stable.** Code and env vars reference aliases,
   ...
   rotation operator-friendly. The known caveat in §3.2.5 is the
   one exception — track via `kms2`.
...
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| `kms2`     | `AwsKmsAdapter.unwrapDataKey` pins `KeyId` in `kms.Decrypt`. After CMK identity rotation (alias-swap), historical envelopes become unreadable. Fix is to either omit `KeyId` (lose the integrity check) or accept a list of historical data-key ARNs. Routine auto-rotation is unaffected; only manual CMK identity rotation hits this.                                                                                                                     | Security |
